...
const TVShowsPage = () => {
  ...
  const isComingSoon = (show) => {
    return show?.isComingSoon === true || show?.isComingSoon === "true";
  };
        ...
        {filteredShows.map((show) => {
          ...
          return (
            <div 
              key={show._id} 
              onClick={() => {
                if (isComingSoon(show)) {
                  alert("This content is coming soon! 🎬 Please check back shortly.");
                  return;
                }
                navigate(`/tv-shows-episodes/${show._id}`);
              }}
              className="relative bg-white border border-gray-200 hover:border-orange-300 p-4 rounded-2xl shadow-sm flex gap-4 group cursor-pointer transition"
            >
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-md flex-shrink-0 ${
                      isComingSoon(show) ? 'bg-amber-100 text-amber-700' : (displayStatus === 'Ongoing' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700')
                    }`}>
                      {isComingSoon(show) ? 'Coming Soon' : displayStatus}
                    </span>
                  </div>
