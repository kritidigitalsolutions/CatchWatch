...
const MyVideosPage = () => {
  ...
  const isComingSoon = (video) => {
    return video?.isComingSoon === true || video?.isComingSoon === "true";
  };
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
            {videos.map((video) => {
              ...
              return (
                ...
                >
                  {isComingSoon(video) && (
                    <span className="absolute top-3 right-3 z-20 bg-amber-500 text-[8px] text-white font-black tracking-widest px-2 py-0.5 rounded shadow-md uppercase">
                      Coming Soon
                    </span>
                  )}
                  {/* Thumbnail Image */}
