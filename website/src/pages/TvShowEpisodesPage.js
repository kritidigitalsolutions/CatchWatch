...
const TvShowEpisodesPage = () => {
    ...
    const isComingSoon = (ep) => {
        return ep?.isComingSoon === true || ep?.isComingSoon === "true";
    };
                ...
                <div className="flex flex-col gap-4">
                    {episodes.map((ep) => (
                        <div
                            ...
                            onClick={() => {
                                if (isComingSoon(ep)) {
                                    alert("This episode is coming soon! 🎬 Please check back shortly.");
                                    return;
                                }
                                navigate(`/watch-episode/${ep._id}`);
                            }} 
                            className="bg-white border border-gray-200 hover:border-brand-orange/50 hover:shadow-md rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row gap-4 sm:gap-6 group cursor-pointer transition-all duration-300 relative"
                        >
                            {/* Episode Thumbnail */}
                            <div className="w-full sm:w-48 md:w-56 aspect-video bg-neutral-900 rounded-xl overflow-hidden relative flex-shrink-0 shadow-inner">
                                {isComingSoon(ep) && (
                                    <span className="absolute top-2 right-2 bg-amber-500 text-[8px] text-white font-black px-1.5 py-0.5 rounded shadow-sm uppercase z-10">
                                        Soon
                                    </span>
                                )}
                                <img
