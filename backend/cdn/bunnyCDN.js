...
const getConfig = () => {
  ...
  // Fail-fast for common misconfigurations (but don't print secrets)
  const missing = [];
  if (!accessKey) missing.push("BUNNY_ACCESS_KEY");
  if (!storageZone) missing.push("BUNNY_STORAGE_ZONE");

  // CDN URL is only required to build public URLs; upload can still work without it,
  // but discovery / app code typically expects it. We'll validate it too.
  if (!cdnUrl) missing.push("BUNNY_CDN_URL");

  if (missing.length) {
    // Keep this error actionable.
    throw new Error(
      `Missing/invalid Bunny CDN env vars: ${missing.join(", ")}. ` +
        `Check .env and ensure server restart. ` +
        `Note: don't include quotes around values.`
    );
  }
